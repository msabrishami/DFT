...
def obejective(d_frontie, circuit_value,f_idx,f_val,nodelist_test):
    if (circuit_value[f_idx].x == 1):
        ...
        return pair
    biggest = nodelist_test[d_frontie[0]].lev
    ID = d_frontie[0]
    for i in d_frontie:
        if (nodelist_test[i].lev > biggest):
            biggest = nodelist_test[i].lev
            ID = i
    alter_node_index = ID
...
#-------------------------------backtrace-------------------------------#
# modify backtrace: choose easiest or hardest path to backtrace
def backtrace(start, circuit_value,nodelist_test):
    startcpy = []
    startcpy.append(start[0])
    startcpy.append(start[1])
    a = []  #use for remember the index of j with value x
    #print(startcpy)
    while (nodelist_test[startcpy[0]].gtype != 'IPT'):
        i = inversion[gtype[nodelist_test[startcpy[0]].gtype].value]
        if (nodelist_test[startcpy[0]].gtype == 'OR') or (nodelist_test[startcpy[0]].gtype == 'NAND'):
            if (startcpy[1] == 0):
                nodelist_test[startcpy[0]].unodes.sort(key=lambda x: x.lev)
                for j in (nodelist_test[startcpy[0]].unodes):
                    if (circuit_value[j.index].x == 1):
                        startcpy[0] = j.index
                        break
            elif (startcpy[1] == 1):
                nodelist_test[startcpy[0]].unodes.sort(key=lambda x: x.lev, reverse = True)
                for j in (nodelist_test[startcpy[0]].unodes):
                    if (circuit_value[j.index].x == 1):
                        startcpy[0] = j.index
                        break
        elif (nodelist_test[startcpy[0]].gtype == 'NOR') or (nodelist_test[startcpy[0]].gtype == 'AND'):
            if (startcpy[1] == 0):
                nodelist_test[startcpy[0]].unodes.sort(key=lambda x: x.lev, reverse = True)
                for j in (nodelist_test[startcpy[0]].unodes):
                    if (circuit_value[j.index].x == 1):
